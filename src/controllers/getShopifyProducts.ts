...
export async function getShopifyProducts(req: Request, res: Response) {
  try {
    ...
    const products = (data as any).products as any[] | undefined;

    // If no products, just clear table and return
    if (!products || products.length === 0) {
      await db.query('TRUNCATE "ProductVariant"');
      return res.json({ productsFetched: 0, variantsSaved: 0 });
    }

    // 2. Flatten all variants with product + variant titles
    type VariantRow = {
      id: string;
      productId: string;
      productTitle: string;
    ...
    for (const v of variants) {
      ...
      values.push(
        ...
        v.price,             // "price" TEXT
        v.inventoryQuantity, // "inventoryQuantity" INT
      );
