...
export async function syncShopifyOrders(req: Request, res: Response) {
  try {
    const url = new URL(
      `https://${SHOPIFY_STORE_DOMAIN}/admin/api/2024-10/orders.json`
    );
    // tweak as you like
    ...
    await db.query(`TRUNCATE "OrderLineItem"`);
    ...
    for (const order of orders) {
      ...
      for (const li of lineItems) {
        const isCustomSale =
          ...
          li.properties.some(
            (p: any) =>
              p.name === '_shopify_item_type' && p.value === 'custom_sale'
          );
